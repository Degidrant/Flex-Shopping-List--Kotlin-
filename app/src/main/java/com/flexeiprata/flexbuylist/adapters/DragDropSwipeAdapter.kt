...
abstract class DragDropSwipeAdapter(private val context: Context) : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN, ItemTouchHelper.RIGHT or ItemTouchHelper.LEFT) {
    //Целевой ViewHolder должен реализовывать интерфейс [DragDropSwipeAdapter.ItemDragDropMoveViewHolder]

    private val iconDelete = drawableToBitmap(ContextCompat.getDrawable(context, R.drawable.ic_delete)!!)!!
    private val iconEdit = drawableToBitmap(AppCompatResources.getDrawable(context, R.drawable.ic_edit)!!)!!

    private val pDelete = Paint().apply {
        color = ContextCompat.getColor(context, R.color.red_acc)
    }
    private val pEdit = Paint().apply {
        color = ContextCompat.getColor(context, R.color.wild_yellow)
    }
    ...
    override fun onChildDraw(
        ...
        isCurrentlyActive: Boolean
            ...
            if (dX > 0) {
                ...
                c.drawBitmap(
                    iconDelete,
                    itemView.left.toFloat() + convertDpToPx(context, 16),
                ...
                c.drawBitmap(
                    iconEdit,
                    itemView.right.toFloat() - convertDpToPx(context,16) - iconEdit.width,
